...
golden_mean = (sqrt(5) - 1.0) / 2.0  # Aesthetic ratio 黄金分割比例
fig_width = fig_width_pt * inches_per_pt  # width in inches 宽度（英寸）
fig_height = fig_width * golden_mean  # height in inches 高度（英寸）
...
for index in range(4, 11):  # 统计 2013-11-4（周一） 至 2013-11-10（周日）的数据，也就是分析一周的数据
    filepath = os.path.join(datadir, file_name) + str(index).zfill(2) + '.txt'
    sliceSum2 = pd.read_csv(filepath,
                            sep='\t',  # 按tab分割
                            encoding="utf-8-sig",
                            names=['CellID', 'datetime', 'countrycode', 'smsin', 'smsout', 'callin', 'callout',
                                   'internet'],
                            parse_dates=['datetime'],  # 要解析的数据
                            date_parser=parse)  # 使用的解析方法
...
# Z-score
sliceSum_z = (sliceSum_city - sliceSum_city.mean()) / sliceSum_city.std()  # (sliceSum_city - 均值)/标准差
